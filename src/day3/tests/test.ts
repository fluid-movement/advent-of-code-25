...
import { type Result } from "../functions.ts";
...
BATTERIES.forEach(battery => {
  Deno.test(`day3 - step1 - gets correct joltage for battery ${battery.label}`, () => {
...
BIG_BATTERIES.forEach(battery => {
  Deno.test(`day3 - step2 - gets correct big joltage for battery ${battery.label}`, () => {
    assertEquals(f.getJoltage(battery.label, 12), battery.joltage);
  });
});

Deno.test("day3 - step1 - calculates correct result for test", () => {
  const INPUT = u.readLines("day3/input/step1.txt");
  assertEquals(f.part1(INPUT), 357);
});

Deno.test("day3 - step2 - calculates correct result for test", () => {
...
Deno.test("day3 - result - calculates correct result", () => {
  assertEquals(f.process("day3/input/input.txt"), {part1: 17359, part2: 172787336861064} as Result);
});
